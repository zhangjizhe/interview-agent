...
for arg in "$@"; do
    case "$arg" in
        ...
        --logs) LOGS=1 ;;
        -h|--help)
            echo "用法: bash deploy.sh [--fresh|--reset|--stop|--logs]"
            echo "  --fresh  删 volumes 重启（清空数据）"
            echo "  --reset  删容器 + volumes + .env 重建"
            echo "  --stop   停掉所有容器"
            echo "  --logs   tail py-api 日志"
            exit 0
            ;;
        *) err "未知参数: $arg"; exit 1 ;;
    esac
...
echo -n "  等待 py-api healthy "
for i in $(seq 1 60); do
    if docker ps --format "{{.Names}}\t{{.Status}}" | grep -q "interview-py-api.*healthy"; then
        echo
        log "  ✅ py-api healthy（${i}s）"
        break
    fi
    echo -n "."
    sleep 2
...
if ! docker ps --format "{{.Names}}\t{{.Status}}" | grep -q "interview-py-api.*healthy"; then
    warn "  ⚠️ py-api 60s 内未 healthy，查看 docker logs interview-py-api"
fi
...
if curl -s -f http://localhost:3002/api/health/ready >/dev/null 2>&1; then
    log "  ✅ /api/health/ready OK（Redis + Milvus 都连上）"
else
...
log "  后端:        http://localhost:3002"
log "  API 文档:    http://localhost:3002/docs"
log "  OpenAPI:     http://localhost:3002/openapi.json"
