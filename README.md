...
> AI 面试 Demo 系统：候选人上传简历 → 智能出题 → 多轮追问 → 自动评分报告
>
> **核心技术**：LangGraph Multi-Agent · 四层记忆 · Prompt Cache 工程 · 混合检索 RAG · Langfuse 全链路可观测
>
> ⚠️ **定位声明**：本项目为 Demo / 学习项目，尚未接入生产流量。所有性能指标均为 Demo 环境下测得，不代表线上表现。
...
### Token 成本优化 — Demo 环境下 55% Token 节省
...
Demo 环境下 50 轮对话测试：**80K token → 35K token，节省 55%**，成本面板接口响应 < 100ms。

> **测试条件**：1000 次重复请求，相同 prompt 模板 + 不同参数，model=gpt-4o-mini
> **Case 分布**：60% 结构化问答、30% 代码生成、10% 文本总结
> ⚠️ 以上为 Demo 环境 benchmark 脚本测得，非线上数据

### Multi-Agent 编排 — Plan-and-Execute 架构

基于 LangGraph StateGraph 实现 5 节点拓扑，相比 ReAct 更适合多步结构化面试场景：
...
- **流式输出**：multi 模式支持 `graph.stream()` SSE 流式渲染；deepagents 模式 stream 走 ChatOpenAI 真流式（deepagents invoke 为非流式调用）
...
L3 长期记忆  Milvus+Mem0  候选人画像双写，自动去重合并，语义召回（Milvus 不可用时降级到 Qdrant）
...
### RAG 混合检索 — Demo 环境下 24 Case P@5 = 1.0
...
**Demo 环境下 24 个测试用例，P@5=1.0，MRR=1.0，Recall=1.0**

> ⚠️ 以上为 24 case 自测指标，非线上数据
...
| 长期记忆 | Mem0 Cloud / OSS | 自动去重合并候选人画像（默认关闭，需配置 `MEM0_API_KEY`）|
| 向量库 | Qdrant 1.18 + Milvus 2.4 | 双引擎：Qdrant 轻量 KB + Milvus 商用重型（Milvus 不可用时降级到 Qdrant）|
...
AGENT_ENGINE=deepagents   # LangChain DeepAgents（stream 走 ChatOpenAI 真流式，invoke 走 deepagents 图）
...
│   │       │   │   ├── deepagents-agent.service.ts     ★ DeepAgents 引擎
...
│   │       │   │   └── nodes/                          ★ 5 节点实现
│   │       │   ├── interview/
│   │       │   │   ├── controllers/hitl.controller.ts  ★ HITL 审批接口
│   │       │   │   └── services/
│   │       │   │       ├── dynamic-task-queue.service.ts  ★ 动态任务队列
│   │       │   │       ├── hitl.service.ts             ★ HITL 审批服务
...
### RAG 召回基准（Demo 环境，24 Case P@5 = 1.0）
...
> ⚠️ 24 case 自测指标，非线上数据

### Token 成本基准（Demo 环境，50 轮对话）
...
> ⚠️ benchmark 脚本测得，非线上数据

---

## 演进路线

> 下表是"**当前状态 → 下一阶段**"的演进图，与 commit 历史对齐。
> ✅ = 已实现　🚧 = 规划中 / 开发中

| 维度 | 当前状态 | 下一阶段 | 相关文件 |
|------|---------|---------|---------|
| **安全** | ✅ JWT 认证（`/auth/login` + `Authorization: Bearer`）+ Rate Limiting（60/min/IP，demo 模式自动 mock userId） | 🚧 完整 OAuth2 + 角色权限（面试官/管理员/候选人） | [auth.service.ts](apps/api/src/modules/auth/auth.service.ts) |
| **测试** | ✅ 34/34 全过（jest 12 + node:test 22），覆盖 Prompt Cache 三段识别 / JSON 容错解析 / fnv1a 稳定哈希 | 🚧 补前端 Vitest（`apps/web`）+ Playwright e2e（SSE 流、评分报告） | [smoke.spec.ts](apps/api/src/__tests__/smoke.spec.ts) |
| **可观测** | ✅ Langfuse 三级采样：trace 10% / span 50% / generation 100%（成本数据必须完整）。自建成本面板 < 100ms | 🚧 接入 Sentry 异常告警。trace 采样率上线后根据流量回调 | [langfuse.service.ts](apps/api/src/infra/langfuse/langfuse.service.ts) |
| **Agent 引擎** | ✅ LangGraph 5 节点 Supervisor + 动态任务队列 Agent 决策（`agentDecide()` 语义驱动追问/进阶，非规则阈值） | 🚧 Multi-Agent Handoffs（LangGraph Command 原语，Planner 决策路由到 Specialist Agent） | [multi-agent.service.ts](apps/api/src/modules/agent/multi-agent.service.ts) |
| **HITL** | ✅ HITL 基础版（Redis pending 状态 + approve/reject API） | 🚧 LangGraph interrupt 接入前端，HR 可在多轮面试中实时审批 / 否决 | [hitl.service.ts](apps/api/src/modules/interview/services/hitl.service.ts) |
| **流式** | ✅ SSE（`graph.stream()` → `processMessage()`）。LlmGatewayChatModel adapter 使 multi 模式也经过 P0 缓存层 | 🚧 双通道：SSE 主对话 + WebSocket 推送业务事件（系统状态、MCP 工具响应） | [interview.controller.ts](apps/api/src/modules/interview/interview.controller.ts) |
| **MCP 协议** | ✅ 标准 MCP stdio Server（`@modelcontextprotocol/sdk`）。bocha_search / memory_recall / knowledge_bank 三个工具已接入 McpRegistry + McpAdapter | 🚧 外部 MCP Server（GitHub/GitLab/文件系统）spawn 集成 + 健康检查 | [mcp-adapter.service.ts](apps/api/src/modules/mcp/mcp-adapter.service.ts) |
...
1. **生产流量 = 0**：本项目为 Demo / 学习项目，未经生产验证
2. **测试覆盖有限**：34 个烟囱测试，无 e2e 测试
3. **deepagents stream**：stream 模式绕回 ChatOpenAI 真流式，deepagents invoke 为非流式调用
4. **Mem0 默认关闭**：需配置 `MEM0_API_KEY` 才启用
5. **HITL 未接 interrupt**：基础版 HITL 仅支持 Redis pending + API 审批，未接入 LangGraph interrupt 流程
6. **Multi-Agent Handoffs 未实现**：当前 Planner → Executor 为线性执行，未实现 Command 原语路由到 Specialist Agent
