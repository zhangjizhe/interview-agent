...
> AI 结构化面试系统：候选人上传简历 → 智能出题 → 多轮追问 → 自动评分报告
>
> **核心技术**：LangGraph Multi-Agent · HITL 中断审批 · Specialist Handoffs · 四层记忆 · Prompt Cache 工程 · 混合检索 RAG · Langfuse 全链路可观测
...
### Multi-Agent 编排 — Plan-and-Execute + HITL + Handoffs

基于 LangGraph StateGraph 实现 5+2 节点拓扑，相比 ReAct 更适合多步结构化面试场景：
...
| 长期记忆 | Mem0 Cloud / OSS | 自动去重合并候选人画像（需配置 `MEM0_API_KEY`）|
...
### 5. HITL 中断审批（`agents/multi-agent/graph.ts` + `hitl.controller.ts`）
...
### 6. Specialist Handoffs（`agents/multi-agent/nodes/executor.ts`）
...
### 7. JSON 容错解析（`common/json-extract.ts`，22 单测）
...
| 维度 | 当前状态 | 下一阶段 | 相关文件 |
|------|---------|---------|---------|
| **安全** | ✅ JWT 认证（`/auth/login` + `Authorization: Bearer`）+ Rate Limiting（60/min/IP） | 🚧 完整 OAuth2 + 角色权限（面试官/管理员/候选人） | [auth.service.ts](apps/api/src/modules/auth/auth.service.ts) |
...
| **Agent 引擎** | ✅ LangGraph 5+2 节点 Supervisor + HITL interrupt + Specialist Handoffs（interviewer/evaluator/searcher/general） | 🚧 外部 Agent 集成（GitHub/GitLab 工具 Agent） | [multi-agent.service.ts](apps/api/src/modules/agent/multi-agent.service.ts) |
| **HITL** | ✅ LangGraph interrupt + Command(resume) + 前端审批面板 + Redis 状态同步 | 🚧 多级审批流（HR → 主管 → 系统自动） | [hitl.controller.ts](apps/api/src/modules/interview/controllers/hitl.controller.ts) |
...
1. **测试覆盖有限**：34 个烟囱测试，无 e2e 测试
2. **deepagents stream**：stream 模式绕回 ChatOpenAI 真流式，deepagents invoke 为非流式调用
3. **Mem0 需配置**：需配置 `MEM0_API_KEY` 才启用长期记忆
4. **Milvus 降级**：Milvus 不可用时降级到 Qdrant，双引擎优势受限
